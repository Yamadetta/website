<template>
  <main>
    <basic-card class="welcome">
      Добро пожаловать на мой сайт. Меня зовут Игорь и
      <h1>я Frontend-разработчик.</h1>
      Список моих работ можно посмотреть на
      <router-link class="link" to="/projects">странице проектов</router-link>.
    </basic-card>
    ...
    <basic-card>
      ...
      <section class="projects-card" v-if="!projects.isLoading">
        <project-item
          v-for="(project, index) in projects.value"
          :key="project.id"
    ...
    <basic-card>
      ...
      <section class="articles" v-if="!articles.isLoading">
        <article-item
          v-for="(article, index) in articles.value"
          :key="article.id"
          :article="article"
          data-aos="fade-right"
          :data-aos-delay="index * 100"
    ...
    <basic-card class="contacts">
      ...
      <p>Если вы хотите со мной сотрудничать, то напишите мне.</p>
      <ul>
        <li>
          <a
            class="link"
            href="tg://resolve?domain=goodstoryman"
            rel="noopener noreferrer"
            >Telegram</a
          >
        </li>
        <li>
          <a class="link" href="mailto:contact@yamadetta.ru"
            >contact@yamadetta.ru</a
          >
        </li>
      </ul>
    </basic-card>
...
export default {
  ...
  mounted() {
    (async () => {
      ...
      this.projects.value = response
        .slice(response.length - 3, response.length)
        .sort((project1, project2) => (project1.id > project2.id ? -1 : 1));
